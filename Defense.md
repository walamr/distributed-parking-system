...
### 4.9 Recommender TLS, HMAC, and Input Validation

The Assignment 3 recommender socket protocol was updated in `apps/recommender-server/src/main/java/edu/kinneret/parking/recommender/RecommenderServer.java`. Recommender listeners now use `SSLServerSocket`, client and peer calls use `SSLSocket`, TLS is restricted to TLS 1.2 or newer, and server-to-server recommender communication requires mTLS through the configured keystore and truststore. Plain socket clients cannot complete the TLS handshake and are rejected.

Every recommender request and response now includes `type`, `spaceId`, `correlationId`, `timestamp`, `nonce`, `nodeId`, and `hmac`; responses also include `status`, `result`, `reason`, or `localResult` as appropriate. The server rejects missing or invalid HMACs, replayed nonces, stale timestamps older than 60 seconds, malformed JSON, unexpected fields, non-numeric parking spaces, and out-of-range parking spaces. Rejection reasons are logged server-side with timestamp, source address where available, receiving node identity, and reason. Client-facing GUI/CLI failures use generic messages such as `Recommendation service temporarily unavailable`.

The customer GUI now exposes an explicit `Recommend Parking` button and validates the parking-space number before sending. The CLI recommendation flow performs the same input checks and sends signed TLS requests.
