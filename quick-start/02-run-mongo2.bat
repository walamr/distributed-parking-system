...
goto WAIT_START
:LOCAL_READY
echo MongoDB Node 2 is reachable.
echo Configuring MongoDB Node 2 with its network address %MONGO2_IP%...
powershell -NoProfile -ExecutionPolicy Bypass -File ".\setup-mongo-node.ps1" -ContainerName mongo2 -IpAddress "%MONGO2_IP%"
if errorlevel 1 goto FAILED
echo Waiting for MongoDB Node 1 to initialize the fresh replica set...
set /a ATTEMPTS=0
:WAIT_RS
docker exec mongo2 mongosh --host localhost --port 27017 -u mulligan_db_admin -p db_pwd_rotated_admin --authenticationDatabase admin --tls --tlsAllowInvalidCertificates --tlsAllowInvalidHostnames --tlsCAFile /etc/mongo/certs/ca-cert.pem --tlsCertificateKeyFile /etc/mongo/certs/mongo2.pem --quiet --eval "const s=rs.status(); const bad=s.members.filter(function(m){return [1,2].indexOf(m.state)===-1;}); if(s.ok===1){if(s.members.length===3){if(bad.length===0){quit(0);}}} quit(2);" >nul 2>&1
