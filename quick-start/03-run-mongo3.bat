...
goto WAIT_START
:LOCAL_READY
echo MongoDB Node 3 is reachable. Checking replica-set health...
set /a ATTEMPTS=0
:WAIT_RS
docker exec mongo3 mongosh --tls --tlsAllowInvalidCertificates --tlsAllowInvalidHostnames --tlsCAFile /etc/mongo/certs/ca-cert.pem --tlsCertificateKeyFile /etc/mongo/certs/mongo3.pem --host localhost --port 27017 --eval "const s=rs.status(); if (!s.ok || s.members.length ^< 3 || s.members.some(m =^> m.state !== 1 ^&^& m.state !== 2)) { quit(2); }" --quiet >nul 2>&1
