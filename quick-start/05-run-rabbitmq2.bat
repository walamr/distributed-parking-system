...
echo Stopping RabbitMQ Node 2 container...
echo Persistent Docker volume rabbitmq2-data is kept. No RabbitMQ data is deleted.
docker compose --env-file network-ips.env -f docker-compose.rabbitmq2.yml down
exit /b %ERRORLEVEL%

:START
call :CHECK_DOCKER
if errorlevel 1 goto FAILED
echo Safe startup restarts the RabbitMQ Node 2 container without deleting the persistent volume.
call :CLEAN
if errorlevel 1 goto FAILED
echo Starting RabbitMQ Node 2 container...
docker compose --env-file network-ips.env -f docker-compose.rabbitmq2.yml up -d
if errorlevel 1 goto FAILED
call :WAIT_READY rabbitmq2
if errorlevel 1 goto FAILED

docker exec rabbitmq2 rabbitmqctl cluster_status 2>nul | findstr /c:"rabbit@rabbitmq1" >nul
if not errorlevel 1 goto ALREADY_JOINED
echo Joining RabbitMQ Node 2 to rabbit@rabbitmq1...
docker exec rabbitmq2 rabbitmqctl stop_app || goto JOIN_FAILED
docker exec rabbitmq2 rabbitmqctl reset || goto JOIN_FAILED
docker exec rabbitmq2 rabbitmqctl join_cluster rabbit@rabbitmq1 || goto JOIN_FAILED
docker exec rabbitmq2 rabbitmqctl start_app || goto JOIN_FAILED
goto JOINED
:ALREADY_JOINED
echo RabbitMQ Node 2 is already joined to Node 1; reset was skipped.
:JOINED
echo RabbitMQ Node 2 is ready.
...
echo one of Nodes 1 or 3 is running and healthy.
echo.
...
call :TRY_RECOVER rabbitmq3
if not errorlevel 1 goto RECOVER_DONE
echo ERROR: Could not rejoin via Node 1 or Node 3.
echo Confirm at least one of them is running and reachable, then try again.
docker exec rabbitmq2 rabbitmqctl start_app >nul 2>&1
goto FAILED
:RECOVER_DONE
echo.
echo SUCCESS: RabbitMQ Node 2 rejoined the cluster.
...
set /a FATT=0
:TRY_RECOVER_FORGET
docker exec rabbitmq2 rabbitmqctl -n rabbit@%~1 forget_cluster_node rabbit@rabbitmq2 >nul 2>&1
...
if errorlevel 1 exit /b 1
docker exec rabbitmq2 rabbitmqctl join_cluster rabbit@%~1
if errorlevel 1 exit /b 1
...
exit /b 0
:FAILED
echo RabbitMQ Node 2 operation failed. Confirm Node 1 is running and reachable at RABBIT1_IP.
pause
goto MENU
